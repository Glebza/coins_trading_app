...
def handle_socket_message(ws, msg):
    ...
    if is_candle_closed:
        ...
        macd, signal, diff = talib.MACD(np_closes,fastperiod = 12, slowperiod=26, signalperiod=9)
        prev_rsi = rsi[-2]
        ...
        print('macd {}'.format(macd[-1]))
        print('diff {}'.format(diff[-1]))
        print('signal {}'.format(signal[-1]))
        print('macd upcross? {}'.format(macd[-1] > signal[-1] and macd[-2] <= signal[-2]))
        print("----")
