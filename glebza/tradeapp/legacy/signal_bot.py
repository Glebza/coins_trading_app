"""Binance futures signal scanner (legacy)."""
...
logging.basicConfig(
    format="%(levelname)s: %(asctime)s %(name) at line %(lineno)  %(message)s",
    level=logging.INFO,
)
...
tickers_data = {
    item["symbol"]: {"24h_price_change": float(item["priceChangePercent"]), "dt": datetime.datetime.now()}
    for item in response.json()
    if "USDC" not in item["symbol"]
}
...
with ThreadPoolExecutor(max_workers=6) as executor:
    results = list(
        signal
        for signal in executor.map(service.check_for_signal, tickers_data.keys(), tickers_data.values())
