...
class StrategyBackTest:
    def __init__(self, strategy, args):
        ...
        last_order = self.repository.get_last_order_id(args.symbol)[0]
        self.order_id = 1
        if last_order:
            self.order_id += last_order
        self.start_cash = args.start_cash
        self.kline_interval = args.kline_interval
        self.backtest_start_date = args.backtest_start_date
        self.backtest_end_date = args.backtest_end_date
        self.backtest_interval = args.backtest_interval

    def test_strategy(self):
        new_launch = self.backtest_launch_repo.add_backtest_launch(
            launch_dtm=datetime.now(),
            backtest_start=self.backtest_start_date,
            backtest_end=self.backtest_end_date,
        ...
        if self.backtest_interval == "day":
            ...
            while start_date < backtest_end_dtm:
                ...
                self.lows = []
                self.intervals = []
                klines = self.repository.get_historical_klines(start_date, end_date, self.symbol, self.kline_interval)
                ...
                for kline in klines:
                    ...
                    track = self.strategy.process([self.close_prices, self.highs, self.lows], self.intervals, buy_order)
                    ...
                    if track['action'] == ACTION_SELL:
                        ...
                        close_orders.append(service.close_deal(self.client, self.symbol, closed_price, buy_order))
                        buy_order = None
                ...
                if buy_order:
                    ...
                    close_orders.append(
                        service.close_deal(self.client, self.symbol, self.close_prices[-1], buy_order))
                    buy_order = None
